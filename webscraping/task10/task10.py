...
def get_movie_list_details(u): # creat a json file in which detail off all movies are store 
    data={}
    c=1
    for i in u['Data']:
        dd={}
...
def analyse_language_and_directors():
    try:
        ...
        file.close()
        try:
            fff=open("movielist.json",'r')
            fff.close()
        except:
            get_movie_list_details(uu)
        finally:
            fff=open("movielist.json",'r')
            di=json.load(fff)
            fff.close()
    ...
    for i in di.values():
        ...
        for j in dicy:
            s_d.add(j)
    d_l={}  #director with language count
        
        
    a=list(s_d)
    # exit()
    for jj in a:
        lang_c={} # language counter dictonary
        for i in di.values():
            if jj in i['Director']:
                for ad in i["Language"]:
                    if ad in lang_c:
                        lang_c[ad]=lang_c[ad]+1
                    else:
                        lang_c[ad]=1
        d_l[jj]=lang_c
    print(json.dumps(d_l,indent=4))
    fi=open("director_language.json",'w')
    json.dump(d_l,fi,indent=8)
